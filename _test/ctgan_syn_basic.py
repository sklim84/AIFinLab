...
def degree_to_distribution(degree_values):
    ...
    print(f'first 5 degress are {dist[:20]}')
    sorted_dist = sorted(dist, reverse=True)
    print(f'정렬된 첫 5개의 값: {sorted_dist[:20]}')
    return dist
...
for model_item in models:
    ...
    for n_gen_samples in n_gen_samples_arr:
        ...
        synthetic_data.to_csv(f'./results/hf_{model_name[i]}_syn_{n_gen_samples}KG_basic_02.csv', index=False)
