...
# 샘플 데이터를 데이터프레임으로 생성
trans = pd.read_csv("C:\\Users\\fuser03\\CD_TRNS_TRAN.csv")
df = pd.DataFrame(trans)

'''
print("데이터 연도:")
df['tran_year'] = df['tran_dt'].astype(str).str[:4]
unique_years = df['tran_year'].unique()
print(unique_years)
...
# 거래 시간대별 거래금액 분포 시각화
plt.figure(figsize=(10, 6))
sns.boxplot(x='tran_tmrg', y='tran_amt', data=df)
plt.title('Transaction Amount by Time Range')
plt.xlabel('Transaction Time Range')
plt.ylabel('Transaction Amount')
plt.show()
...
count_table = filtered_df.groupby(['tran_code', 'ff_sp_ai']).size().unstack(fill_value=0)
...
# 'fnd_type' 별로 'ff_sp_ai' 값의 누적 갯수 계산
count_table = filtered_df.groupby(['hdl_fc_sn', 'ff_sp_ai']).size().unstack(fill_value=0)
