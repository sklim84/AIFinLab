...
def analyze_label_centrality(data, label_column, label_value):
    filtered_data = data[data[label_column] == label_value]
    G = create_network(filtered_data)
    centrality_measures = calculate_centrality_measures(G)
    return centrality_measures


# Centrality 계산
def calculate_centrality_measures(G, max_iter=1000, tol=1e-06):
...
def main():
    ...
    print("1")
    # ff_sp_ai 열의 null 값을 '00'으로 치환
    original_data['ff_sp_ai'].fillna('00', inplace=True)
    # FF_SP_AI 레이블 목록
    ff_sp_ai_labels = original_data['ff_sp_ai'].unique()
    # 원본 데이터 레이블 별 중심성 계산
    original_label_centrality_measures = {label: analyze_label_centrality(original_data, 'ff_sp_ai', label) for label in
                                          ff_sp_ai_labels}
