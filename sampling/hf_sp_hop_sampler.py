...
df_hf_trns_tran = df_hf_trns_tran.astype(
    {'wd_fc_sn': 'str', 'wd_ac_sn': 'str', 'dps_fc_sn': 'str', 'dps_ac_sn': 'str', 'tran_amt': 'float'})
df_hf_trns_tran['WD_NODE'] = df_hf_trns_tran[['wd_fc_sn', 'wd_ac_sn']].apply('-'.join, axis=1)
df_hf_trns_tran['DPS_NODE'] = df_hf_trns_tran[['dps_fc_sn', 'dps_ac_sn']].apply('-'.join, axis=1)
df_hf_trns_tran.to_csv(f'{data_name}_ST.csv', index=False)

# Default information : 'TR' - # transactions, 'AL' - 01+02+SP, 'AC' - # accounts
df_hf_trns_tran = pd.read_csv(f'./{data_name}_ST.csv', dtype=object)
count_tran = len(df_hf_trns_tran)
anomaly_count = df_hf_trns_tran['ff_sp_ai'].value_counts()
count_01 = anomaly_count.get('01', 0)
count_02 = anomaly_count.get('02', 0)
count_SP = anomaly_count.get('SP', 0)

# 입출금 계좌 리스트업 : 중복 제거
src_acc = df_hf_trns_tran['WD_NODE'].to_numpy()
tar_acc = df_hf_trns_tran['DPS_NODE'].to_numpy()
...
df_anomaly_tran_h0 = df_hf_trns_tran.loc[df_hf_trns_tran['ff_sp_ai'].isin(anomaly_labels)]
...
# df_anomaly_tran_h1 = df_hf_trns_tran.loc[
#     df_hf_trns_tran['WD_NODE'].isin(anomaly_acc_h0) | df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h0)]
df_anomaly_tran_h1 = df_hf_trns_tran.loc[df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h0)]
df_anomaly_tran_h1.to_csv(f'{data_name}_{"_".join(anomaly_labels)}_H1.csv', index=False)
count_tran_h1 = len(df_anomaly_tran_h1)

# 거래 내 계좌 리스트업
src_acc_h1 = df_hf_trns_tran.loc[df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h0)]['WD_NODE'].to_numpy()
# tar_acc_h1 = df_hf_trns_tran.loc[df_hf_trns_tran['WD_NODE'].isin(anomaly_acc_h0)]['DPS_NODE'].to_numpy()
...
# df_anomaly_tran_h2 = df_hf_trns_tran.loc[
#     df_hf_trns_tran['WD_NODE'].isin(anomaly_acc_h1) | df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h1)]
df_anomaly_tran_h2 = df_hf_trns_tran.loc[df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h1)]
df_anomaly_tran_h2.to_csv(f'{data_name}_{"_".join(anomaly_labels)}_H2.csv', index=False)
count_tran_h2 = len(df_anomaly_tran_h2)

# 거래 내 계좌 리스트업
src_acc_h2 = df_hf_trns_tran.loc[df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h1)]['WD_NODE'].to_numpy()
# tar_acc_h2 = df_hf_trns_tran.loc[df_hf_trns_tran['WD_NODE'].isin(anomaly_acc_h1)]['DPS_NODE'].to_numpy()
...
# # Extract 3-hop transactions
# # df_anomaly_tran_h3 = df_hf_trns_tran.loc[
# #     df_hf_trns_tran['WD_NODE'].isin(anomaly_acc_h2) | df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h2)]
# df_anomaly_tran_h3 = df_hf_trns_tran.loc[df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h2)]
# df_anomaly_tran_h3.to_csv(f'{data_name}_{"_".join(anomaly_labels)}_H3.csv', index=False)
# count_tran_h3 = len(df_anomaly_tran_h3)
#
# # 거래 내 계좌 리스트업
# src_acc_h3 = df_hf_trns_tran.loc[df_hf_trns_tran['DPS_NODE'].isin(anomaly_acc_h2)]['WD_NODE'].to_numpy()
# # tar_acc_h3 = df_hf_trns_tran.loc[df_hf_trns_tran['WD_NODE'].isin(anomaly_acc_h2)]['DPS_NODE'].to_numpy()
# anomaly_acc_h3 = np.concatenate((anomaly_acc_h2, src_acc_h3))
# anomaly_acc_h3 = np.unique(anomaly_acc_h3)
#
# anomaly_count_h3 = df_anomaly_tran_h3['ff_sp_ai'].value_counts()
# count_01_h3 = anomaly_count_h3.get('01', 0)
# count_02_h3 = anomaly_count_h3.get('02', 0)
# count_SP_h3 = anomaly_count_h3.get('SP', 0)
#
# print('##### H3 Data Information')
# print(f'# \'AC\':\t{anomaly_acc_h3.shape[0]}')
# print(f'# \'TO\':\t{count_tran_h3}')
# print(f'# \'AL\':\t{(count_01_h3 + count_02_h3 + count_SP_h3) * 100 / count_tran_h3} %')
# print(f'# \'01\':\t{count_01_h3 * 100 / count_tran_h3} %')
# print(f'# \'02\':\t{count_02_h3 * 100 / count_tran_h3} %')
# print(f'# \'SP\':\t{count_SP_h3 * 100 / count_tran_h3} %')

df_hf_training = df_anomaly_tran_h2[:10000]
df_hf_training.to_csv(f'../results/hf_training.csv', index=False)
